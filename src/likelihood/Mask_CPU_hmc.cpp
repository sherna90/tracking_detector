...
void Mask_CPU_Hamiltonian_MC::init(MatrixXd &_X, VectorXd &_Y, double _lambda, int _warmup_iterations, int _iterations, double _step_size, int _num_step, double _path_length){
    ...
    this->logistic_regression = Mask_CPU_LogisticRegression(_X, _Y, this->lambda, false, true, true);
    this->init = true;
    this->sampled = 0.0;
    this->accepted = 0.0;
...
void Mask_CPU_Hamiltonian_MC::loadModel(VectorXd weights, VectorXd featureMean, VectorXd featureStd, VectorXd featureMax, VectorXd featureMin, double bias){
	this->logistic_regression = Mask_CPU_LogisticRegression(false, true, true);
